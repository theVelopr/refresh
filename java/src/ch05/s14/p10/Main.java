...
public class Main {
    public static void main(String[] args) throws InterruptedException {
        ...
        // 4. 스레드 종료 (동기화)
        // 스레드 풀은 자동 종료가 안되기 때문에, 직접 종료해주어야 한다.

        // Thread.join()과 마찬가지로 작업이 끝나기를 기다려서 종료
        pool1.shutdown();

        Thread.sleep(1000);

        // get()은 Blocking method
        // get()이 영원히 기다릴 수 있으므로 timeout설정 가능
//        System.out.println(future.get());

        future.cancel(true);

        // cancel을 호출하고 나면 get을 호출할 수 없다. (CancellationException) 발생
        // mayInterruptIfrunning이 true이면, 스레드에 interrup도 발생시킨다.
        try {
            future.get();
