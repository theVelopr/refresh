...
// 여러개의 타입 파라미터도 쓸 수 있다.
class HasMap<K,V> {}

class GenericBar <T> {
    // 문법적으로 문제가 있는 경우
    // static T classVar; // impossible
    // static void method(T var) {} // impossible

    // 문법적으로 문제가 없을 것 같으나, 안정성 문제로 사용 금지
    // T memberVar = new T(); -> 안정성 문제 때문에 불가능
    /*
        Object obj = new Object();
        if (obj instanceof T) {} // impossible
     */
}
...
class GIFoo<K, T, D> extends GFoo<T> implements IGFoo<D> {

}

// 부모 클래스/인터페이스들에 동일한 타입 파라미터를 넘겨줄 수 있다.
class IGFooTwo<T> extends GFoo<T> implements IGFoo <T> {
...
class WildCard {
    ...
    public void method1_eq(WildGeneric<? extends Object> x) {} // Object가 상속한 -> 모든 클래스
    public void method2(WildGeneric<? extends  WildFoo> x) {} // WildFoo, WildBar
    public void method3(WildGeneric<? extends  WildBar> x) {} // Object, WildFoo, WildBar
}
...
public class Generic {
    public static void main(String[] args) {
        ...
        GenericFoo<String> foo1 = new GenericFoo<>("name1", "var1");

        System.out.println(foo.name); // name
        System.out.println(foo.memberVar); // var
        System.out.println(foo1.name); // name1
        System.out.println(foo1.memberVar); // var1
        System.out.println();

        GenericFoo<Integer> foo2 = new GenericFoo<>("name2", 4);
        System.out.println(foo2.name); // name2
        System.out.println(foo2.memberVar); // 4

        // 동적 바인딩 -> 컴파일타임에는 동작이 완전히 정의가 되지 않음
        // 런타임에 결정됨
        GenericMethod.staticMethod("abcd");
        GenericMethod.staticMethod(12321);
