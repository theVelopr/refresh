 ...
 * - 아무런 구현이 되지 않지 않으며, 모든 메소드가 추상 메소드이다.
 *
 * 인터페이스의 특징
 * - class가 아니고 interface 키워드를 사용
 * - 멤버변수는 항상 "public static final"이다. (생략가능)
 * - 멤버메소드는 항상 "public abstract"이다. (생략가능)
...
// public, default 가 가능
interface IFoo {
    public static final int MEMEBR_VAR = 10;
 ...
 *  - 인터페이스의 철학과 맞지 않으나,
 *    기존에 사용하던 언터페이스가 개선되어, 모든 자식 메소드에 동일하게
...
public class Interface {
    public static void main(String[] args) {
        ...
        footwo.defaultMethod(); // Default method
        ...
        // 부모와 인터페이스에 모드 메소드가 있는 경우,
        // 부모 클래스에 잇는 메소드를 실행한다.
        // 다중 상속 시 문제를 방지하기 위해, 인터페이스보다 일반 상속이 우선시

        // 인터페이스명으로 클래스 메소드 호출 가능
        IBar.staticMethod(); // static Method
        // 구현체인 자식 클래스로는 클래스 메소드 호출 불가능
//        Bar.staticMethod();
    }
}
